...
const HotelPage = () => {
    ...
    const { data: hotelData, isLoading: isHotelLoading } = useGetHotelQuery(id || "0");
    const { data: reviewsData, isLoading: isReviewsLoading } = useGetPageReviewsQuery({
        hotelId: Number(id),
    });
    const { data: roomsData, isLoading: isRoomsLoading } = useGetPageRoomsQuery({
        hotelId: Number(id),
    ...
    return (
        <div className="container mx-auto mt-5 grid grid-cols-4 gap-5">
            ...
            {isHotelLoading || isReviewsLoading || isRoomsLoading ? (
                <HotelPageSkeleton />
            ) : (
